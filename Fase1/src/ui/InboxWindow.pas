...
begin
  ...
  FSortButton := TUIUtils.CreateButton('📋 Ordenar por Asunto', @OnSortClicked, Self);
  FRefreshButton := TUIUtils.CreateButton('🔄 Actualizar', @OnRefreshClicked, Self);
  FDeleteButton := TUIUtils.CreateButton('🗑️ Eliminar', @OnDeleteClicked, Self);
  FMarkReadButton := TUIUtils.CreateButton('✅ Marcar como Leído', @OnMarkReadClicked, Self);
...
begin
  ...
  begin
    ...
    if TUIUtils.ShowConfirmDialog(FWindow, 'Eliminar Email',
                                 '¿Está seguro que desea eliminar este email?') then
    begin
...
begin
  ...
  begin
    ...
    begin
      RefreshInbox;
      TUIUtils.ShowInfoMessage(FWindow, 'Email marcado como leído');
    end
    else
    begin
      TUIUtils.ShowErrorMessage(FWindow, 'Error al marcar email como leído');
    end;
  end
  else
  begin
    TUIUtils.ShowErrorMessage(FWindow, 'Por favor seleccione un email para marcar como leído');
  end;
end;
...
begin
  ...
  if Email^.Estado = 'NL' then
    DetailText := DetailText + 'No Leído'
  else
    DetailText := DetailText + 'Leído';
...
begin
  // Esta función se mantiene por compatibilidad, pero la lógica está en ShowEmailDetails
end;
