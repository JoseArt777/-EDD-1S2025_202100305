...
uses
  GTK2, GDK2, SysUtils, Classes, DateUtils, DataStructures, SystemCore, UIBase,
...
begin
  ...
  // Establecer valores por defecto (fecha y hora actual + 1 hora)
  CurrentDateTime := Now + (1/24); // Agregar 1 hora
  DateStr := FormatDateTime('dd/mm/yyyy', CurrentDateTime);
  TimeStr := FormatDateTime('hh:nn', CurrentDateTime);
...
begin
  ...
  // Restablecer fecha y hora por defecto
  CurrentDateTime := Now + (1/24); // + 1 hora
  DateStr := FormatDateTime('dd/mm/yyyy', CurrentDateTime);
  TimeStr := FormatDateTime('hh:nn', CurrentDateTime);
...
begin
  ...
  try
    ...
    Minute := StrToInt(TimeParts[1]);

    // Validar rangos
    if (Day < 1) or (Day > 31) or (Month < 1) or (Month > 12) or
       (Year < 2025) or (Hour < 0) or (Hour > 23) or (Minute < 0) or (Minute > 59) then
      raise Exception.Create('Valores de fecha/hora fuera de rango');
  ...
  begin
    TUIUtils.ShowInfoMessage(FWindow,
      Format('Correo programado exitosamente para %s el %s',
             [Recipient, FormatDateTime('dd/mm/yyyy hh:nn', ScheduleDateTime)]));
