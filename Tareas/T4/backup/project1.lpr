...
uses
  ...
  cthreads,
  {$ENDIF}
  Classes, SysUtils, CustApp, Process;  // ← Agregada unidad Process

type
  { ===== CLASES DEL GRAFO NO DIRIGIDO ===== }

  // Clase para representar una arista con peso opcional
  TArista = class
  private
    FDestino: String;
    FPeso: Integer;
  ...
  public
    ...
    function GenerarImagen(ArchivoDot, ArchivoImagen: String): Boolean;  // ← NUEVO
  end;
...
{ ===== IMPLEMENTACIÓN DE TListaAristas ===== }
...
{ ===== IMPLEMENTACIÓN DE TGrafoNoDirigido ===== }
...
begin
  ...
  // Obtener las listas de adyacencia
  Lista1 := GetListaAristas(Ciudad1);
  Lista2 := GetListaAristas(Ciudad2);

  // Agregar la conexión bidireccional (no dirigida)
...
begin
  ...
  Resultado := Resultado + '  // Configuracion del grafo' + LineEnding;
  ...
  // Usar un stringlist para evitar duplicar aristas
  ...
  try
    ...
    begin
      ...
      for j := 0 to Lista.Count - 1 do
      begin
        // Crear una conexión normalizada (alfabéticamente ordenada)
...
{ ===== NUEVA FUNCIÓN: Generar Imagen PNG ===== }
...
begin
  ...
  WriteLn;
  WriteLn('El programa iniciara un menu interactivo');
  WriteLn('para gestionar un grafo no dirigido.');
end;
...
begin
  ...
  WriteLn('5. Guardar .dot y generar imagen .png');  // ← MODIFICADO
...
begin
  WriteLn;
  WriteLn('Cargando ejemplo del PDF...');
  WriteLn;

  // Agregar ciudades
  FGrafo.AgregarCiudad('A');
  WriteLn('+ Ciudad A agregada');
  FGrafo.AgregarCiudad('B');
  WriteLn('+ Ciudad B agregada');
  FGrafo.AgregarCiudad('C');
  WriteLn('+ Ciudad C agregada');
  FGrafo.AgregarCiudad('D');
  WriteLn('+ Ciudad D agregada');

  WriteLn;

  // Agregar conexiones
  FGrafo.AgregarConexion('A', 'B');
  WriteLn('+ Conexion A -- B agregada');
  FGrafo.AgregarConexion('A', 'C');
  WriteLn('+ Conexion A -- C agregada');
  FGrafo.AgregarConexion('B', 'D');
  WriteLn('+ Conexion B -- D agregada');

  WriteLn;
  WriteLn('¡Ejemplo cargado exitosamente!');
end;
    ...
    case Opcion of
      1: begin
           ...
           begin
             ...
             WriteLn('¡Ciudad "' + Ciudad + '" agregada exitosamente!');
           end;
         end;
      ...
      3: begin
           ...
           begin
             WriteLn('El grafo esta vacio. Agregue ciudades primero.');
           end
           else
           begin
      ...
      4: begin
           ...
           begin
             ...
             WriteLn('Para visualizar el grafo:');
             WriteLn('1. Copie el codigo de arriba');
             WriteLn('2. Vaya a: https://dreampuf.github.io/GraphvizOnline/');
             WriteLn('3. Pegue el codigo y vea el resultado');
           end;
         end;

      5: begin  // ← MODIFICADO: Ahora genera .dot Y .png
           WriteLn;
           if FGrafo.ObtenerCantidadCiudades = 0 then
           begin
             WriteLn('El grafo esta vacio. Agregue ciudades primero.');
           ...
           begin
             try
               ...
               WriteLn('✓ Archivo "grafo.dot" guardado exitosamente!');
               WriteLn;

               // Intentar generar imagen PNG
               WriteLn('Intentando generar imagen PNG...');
               if FGrafo.GenerarImagen('grafo.dot', 'grafo.png') then
               begin
                 WriteLn('✓ ¡Imagen "grafo.png" generada exitosamente!');
                 WriteLn;
                 WriteLn('Puedes ver la imagen con:');
                 WriteLn('  xdg-open grafo.png    (Linux)');
                 WriteLn('  eog grafo.png         (Linux)');
                 WriteLn('  start grafo.png       (Windows)');
               end
               else
               begin
                 WriteLn('✗ No se pudo generar la imagen PNG');
                 WriteLn;
                 WriteLn('Posibles causas:');
                 WriteLn('1. Graphviz no está instalado');
                 WriteLn('   Instalar con: sudo apt-get install graphviz');
                 WriteLn('2. El comando "dot" no está en el PATH');
                 WriteLn;
                 WriteLn('Alternativa:');
                 WriteLn('Ejecuta manualmente: dot -Tpng grafo.dot -o grafo.png');
               end;

               WriteLn;
               WriteLn('=== CONTENIDO DEL ARCHIVO .DOT ===');
               WriteLn(FGrafo.GenerarGraphviz);
               ...
               begin
                 ...
                 WriteLn;
                 WriteLn('=== CONTENIDO QUE SE GUARDARIA ===');
                 WriteLn(FGrafo.GenerarGraphviz);
               end;
             end;
           end;
      ...
      7: begin
           ...
           begin
             ...
             begin
               FGrafo.Free;
               FGrafo := TGrafoNoDirigido.Create;
               WriteLn('¡Grafo limpiado exitosamente!');
             end;
           end
           else
